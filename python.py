...
# Отправитель банковского SMS.
# Твой реальный формат:
#
# From: 5800
#
SMS_BANK_SENDER = os.environ.get(
    "SMS_BANK_SENDER",
    "From: 5800"
).strip()
...
TEXTS = {
    "ru": {
        ...
        "refill_payment": (
            ...
            "<b>{amount:,} сум</b>\n"
            "💳 Перевести нужно ровно: "
            "<b>{payment_amount:,} сум</b>\n\n"
            "Переведите деньги на карту:\n"
            "<code>{card}</code>\n\n"
            "📱 После перевода банковское SMS "
            "автоматически попадёт в систему.\n"
            "🤖 Бот проверит SMS и сам зачислит "
            "<b>{amount:,} сум</b>.\n\n"
            "⚠️ Переводите именно указанную сумму. "
            "Чек отправлять не нужно."
        ),
    ...
    "uz": {
        ...
        "refill_payment": (
            ...
            "💰 Balansga tushadi: "
            "<b>{amount:,} so'm</b>\n"
            "💳 Aynan shuni o'tkazing: "
            "<b>{payment_amount:,} so'm</b>\n\n"
            "Kartaga pul o'tkazing:\n"
            "<code>{card}</code>\n\n"
            "📱 Bank SMS xabari avtomatik tekshiriladi.\n"
            "🤖 Bot <b>{amount:,} so'm</b>ni "
            "avtomatik qo'shadi.\n\n"
...
def create_auto_refill(user_id, requested_amount):
    """
    Создаёт уникальную контрольную сумму.
    ...
    Пользователь получает на баланс requested_amount.
    """

    conn = db()
    cursor = conn.cursor()

    # Отменяем старую активную заявку пользователя.
    cursor.execute(
        """
        UPDATE auto_refills
    ...
    # Контрольная добавка от 10 до 99 сум.
    for _ in range(100):

        suffix = random.SystemRandom().randint(
            10,
            99,
        )
        ...
        logger.info(
            "AUTO REFILL CREATED | "
            "user_id=%s | requested=%s | payment=%s",
            user_id,
            requested_amount,
            payment_amount,
        )
    ...
    raise RuntimeError(
        "Не удалось создать уникальную сумму пополнения"
    )
...
def sms_has_bank_sender(text):
    """
    Проверяет формат отправителя.

    Реальный SMS2 Forwarder:

    From: 5800
    Time: 2026-09-02 19:38:46+0500
    ...
    return first_line in allowed


def extract_sms_amount(text):
    """
    Извлекает ТОЛЬКО сумму перевода из:
    ...
    # Деньги в нашей системе считаются целыми сумами.
...
def process_sms_text(text):
    """
    Автоматически обрабатывает банковский SMS.
    ...
    Kartaga o'tkazma: PAYME P2P, UZ,02.09.26 19:38,
    karta ***3546. Miqdor:4080.00 UZS
    Qoldiq:1029.91 UZS

    Из этого SMS берётся:
    ...
    if not sms_has_bank_sender(text):
        ...
        return {
            "ok": False,
            "reason": "bank_sender_not_found",
        }

    # -----------------------------------------------------
    # 2. Получаем сумму только из Miqdor
    # -----------------------------------------------------

    amount = extract_sms_amount(text)
    ...
    try:
        ...
        # -------------------------------------------------
        # 5. Ищем активную заявку
        # -------------------------------------------------
        #
        # Например:
        #
        # Пользователь попросил 4000.
        #
        # Бот создал:
        #
        # payment_amount = 4080
        ...
        # -------------------------------------------------
        # 6. Подходящей заявки нет
        # -------------------------------------------------

        if row is None:

            logger.info(
                "SMS PAYMENT NOT FOUND | amount=%s",
                amount,
            )

            # Не записываем неизвестное SMS в processed_sms.
            #
            # Это важно: если пользователь создаст заявку
            # немного позже, то это SMS можно будет
            # обработать повторно.
            conn.close()
        ...
        # -------------------------------------------------
        # 10. Сохраняем SMS как обработанное
        # -------------------------------------------------
...
async def sms_forward_message(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
):
    """
    Принимает SMS от SMS2 Forwarder.
    """
    ...
    try:
        ...
        logger.exception(
            "AUTO REFILL USER NOTIFICATION ERROR: %s",
            e,
        )
    ...
    if ADMIN_ID:
        ...
        try:
            ...
            logger.exception(
                "AUTO REFILL ADMIN NOTIFICATION ERROR: %s",
                e,
            )
...
async def buy_confirm(
    ...
    product_type = context.user_data[
        "product_type"
    ]
    ...
    try:
        ...
        logger.exception(
            "ADMIN ORDER NOTIFICATION ERROR: %s",
            e,
        )
...
def main():
    ...
    #
    # ВАЖНО:
    # ЭТОТ HANDLER ДОЛЖЕН БЫТЬ ДО SMS FORWARDER.
    #
    # Иначе сообщение "1000" после кнопки
    # "Пополнить баланс" может перехватить
    # sms_forward_message().
    # =====================================================
